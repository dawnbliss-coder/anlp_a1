...
def parse_args():
    ...
    p.add_argument(
        "--bpe_train_sample", type=int, default=800,
        help="cap on how many train examples the BPE merge-learning step sees (C1-C4 only); "
             "the cipher's ~2000-symbol raw bit-strings make training on the full set too slow, "
             "so merges are learned from a random subsample, then applied to every example",
    )
...
def build_model(cfg, src_vocab_size, tgt_vocab_size, src_pad_id, tgt_pad_id, device):
...
def main():
    ...
    data = build_dataloaders(
        ...
        vocab_size=args.vocab_size, bpe_train_sample=args.bpe_train_sample, seed=args.seed,
    )

    src_pad_id = data.src_tokenizer.pad_id if data.src_tokenizer is not None else 0
    model = build_model(cfg, data.src_vocab_size, data.tgt_vocab_size, src_pad_id, data.pad_id, device)
    ...
    if data.tokenizer is not None:
        data.tokenizer.save(os.path.join(ckpt_dir, "tgt_tokenizer.json"))
    if data.src_tokenizer is not None:
        data.src_tokenizer.save(os.path.join(ckpt_dir, "src_tokenizer.json"))
